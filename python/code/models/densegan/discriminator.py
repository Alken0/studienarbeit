...
def make_discriminator_model() -> Model:
    ...
    # hidden layer 0
    merge = Dense(128, input_shape=(IMG_DIM,), name="input")(merge)
    merge = LeakyReLU(alpha=0.2)(merge)

    # hidden layer 1
    merge = Dense(256, name="hidden1")(merge)
    merge = LeakyReLU(alpha=0.2)(merge)

    # hidden layer 2
    merge = Dense(512, name="hidden2")(merge)
    merge = LeakyReLU(alpha=0.2)(merge)

    # output
    out = Flatten()(merge)
    out = Dropout(DROPOUT)(out)
    out = Dense(1, activation="tanh", name="output")(out)
